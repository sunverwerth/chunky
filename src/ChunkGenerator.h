...
public:
	...
	void init(int sx, int sz) {
		...
		for (int z = sz; z < sz + chunkSize; ++z) {
			for (int x = sx; x < sx + chunkSize; ++x) {
				float scale = pow(perlin.noise0_1(0.0005f*x + start.x, 0.0005f*z + start.y), 4) * 200;
				float ground = -50 + perlin2.noise0_1(0.0005f*x + start.x, 0.0005f*z + start.y) * 100;
				float sharpness = 1.0f + pow(perlin2.noise0_1(0.01f*x + start.x, 0.01f*z + start.y), 4) * 50;
	...
	virtual BlockType getBlockAt(int x, int y, int z) {
		...
		if (y > top) {
			if (y > waterLevel) {
				if (!snow && y == top + 1) {
					float trees = perlin.noise0_1(0.0075f*x + start.x, 0.0075f*z + start.y);
					if (trees > 0.5f) {
						trees = (trees - 0.5f) * 2;
						trees *= 0.1f;
						if ((float)rand() / RAND_MAX < trees) return BlockType::WOOD;
					}
				}
